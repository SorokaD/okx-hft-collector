...
class OrderBookHandler(IOrderBookHandler):
    def __init__(self, storage: IStorage) -> None:
        ...
        self.snapshots: Dict[str, Dict[str, Any]] = {}  # Храним снапшоты по инструментам
    ...
    def _process_snapshot(self, snapshot: Dict[str, Any], inst_id: str) -> Dict[str, Any] | None:
        """Обработка снапшота"""
        try:
    ...
    def _process_increment(self, update: Dict[str, Any], inst_id: str) -> Dict[str, Any] | None:
    ...
    def _parse_price_levels(self, levels: List[List[str]]) -> List[Tuple[float, float]]:
        """Парсинг уровней цен"""
        return [(float(level[0]), float(level[1])) for level in levels if len(level) >= 2]

    def _apply_increments(self, current: List[Tuple[float, float]], increments: List[List[str]]) -> List[Tuple[float, float]]:
    ...
    def _calculate_spread_mid(self, bids: List[Tuple[float, float]], asks: List[Tuple[float, float]]) -> Tuple[float, float]:
